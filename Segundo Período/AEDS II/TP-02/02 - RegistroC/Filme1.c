...
typedef struct {
    ...
    char dataLancamento[100];
    int duracao;
    char genero[100];
    char idiomaOriginal[100];
    char situacao[100];
    float orcamento;
    char palavrasChave[1000][100];
    int numeroPalavras;
...
void ler(char *nomeArquivo,Filme *novoFilme){
    ...
    while((read=getline(&linha,&tamanho,arquivo))!=-1){
        ...
        if (strstr(linha,"<title>")!=NULL){
            ...
            strcpy(novoFilme->nome,stringAux);
            //printf("%s\n",novoFilme.nome);
        }
        ...
        if(strstr(linha,"Palavras-chave")!=NULL){
            ...
            while(true){
                ...
                if(strstr(linha,"/keyword/")!=NULL){
                    ...
                    strcpy(stringAux,substr(stringAux,posicao,strlen(stringAux)-10));
        ...
        if(strlen(novoFilme->tituloOriginal)<2){
            strcpy(novoFilme->tituloOriginal,novoFilme->nome);
        }
    }
    fclose(arquivo);
    if(linha) free(linha); 
