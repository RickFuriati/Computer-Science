...
class pesquisaSequencial {
    ...
    static class Lista {
        ...
        public boolean pesquisar(String nomeFilme){
            boolean resposta=false;
            for(int aux=0;aux<n && resposta==false;aux++){
                //MyIO.println(nomeFilme);
                if (array[aux].getNome().compareTo(nomeFilme)==0){
                    resposta=true;
                    contador++; 
            ...
            try{
                ...
                while(linha!=null){ //Enquanto a linha nao for "null" (fim do arquivo) executa o código
                    ...
                    if(linha.contains("<span class=\"runtime")==true){ //Busca pela tag "runtime" que indica o local onde a duração do filme se encontra no HTML
                        ...
                        if(stringAuxiliar.contains("h")){ //Caso o filme possua duração na casa das horas, trata os dois valores (horas e minutos)
    ...
    public static void main(String[] args) throws Exception{
        ...
        while(true){
            ...
            //resp=listaFilmes.pesquisar(chaveBusca);
            if(listaFilmes.pesquisar(chaveBusca)==true){
                MyIO.println("SIM");
            }else{
                MyIO.println("NAO");
            }
        ...
        Arq.openWrite("752959_sequencial.txt");
        Arq.println("752959\t"+(System.currentTimeMillis()-tempoIni)+" ms\t"+ listaFilmes.getContador()); 
        Arq.close();
    }
}
