...
void selecaoRecursiva(Filme vetor[],int i, int j){
    clock_t inicio=clock();
    int menor=i;
    for (int j=i+1;j<n;j++){    
        if(strcmp(vetor[j].tituloOriginal, vetor[menor].tituloOriginal) < 0){
            menor=j;
            comparacoes++;
        }
    }
    ...
    movimentacoes+=3;
    if(i+1<n){
        selecaoRecursiva(vetor,i+1,n);
    }
    
    FILE *log;
    
    log=fopen("752959_selecaoRecursiva.txt","w+");
