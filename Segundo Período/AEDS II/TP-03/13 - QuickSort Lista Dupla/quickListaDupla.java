...
class quickListaDupla {
    ...
    static class ListaDupla {
        ...
        public void mostrar() {
            ...
            for (CelulaDupla aux = primeiro.prox; aux != null; aux = aux.prox) {
        ...
        public CelulaDupla caminhaPosic(int posic){
           CelulaDupla resp=new CelulaDupla();
           int cont=0;
           for(CelulaDupla i=primeiro.prox;cont<=posic;i=i.prox,cont++){
                resp=i;
           }
           return resp;
        }
        ...
        private void quicksort(int esq, int dir) {
            ...
            //Filme elementoI=celulaI.elemento;
            //Filme elementoJ=celulaJ.elemento;
            //Filme pivo = array[(dir + esq) / 2];
            Filme pivo = caminhaPosic(((dir + esq) / 2)).elemento;
            //MyIO.println(pivo.getSituacao());
            
            while (i <= j) {
                comparacoes++;
                Filme filmeI=caminhaPosic(i).elemento;
                Filme filmeJ=caminhaPosic(j).elemento;
                while (filmeI.getSituacao().compareTo(pivo.getSituacao()) < pivo.getSituacao().compareTo(filmeI.getSituacao())){
                    comparacoes++;
                    celulaI=celulaI.prox;
                    i++;
                }
    
                while (filmeJ.getSituacao().compareTo(pivo.getSituacao()) > pivo.getSituacao().compareTo(filmeI.getSituacao())){
                    comparacoes++;
                    celulaJ=celulaJ.ant;
                    j--;
                ...
                if (i <= j) {
                    //CelulaDupla celulaI=new CelulaDupla(caminhaPosic(i));
                    //CelulaDupla celulaJ=new CelulaDupla(caminhaPosic(j));
        ...
        public void quicksortDesempate() {
            quicksort(0, n - 1);
        }
        ...
        private void quicksortDesempate(int esq, int dir) {
            int i = esq, j = dir;
            CelulaDupla celulaI=caminhaPosic(i);
            CelulaDupla celulaJ=caminhaPosic(j);
            //Filme elementoI=celulaI.elemento;
            ...
            while (i <= j) {
                ...
                Filme filmeI=caminhaPosic(i).elemento;
                Filme filmeJ=caminhaPosic(j).elemento;
                while (filmeI.getNome().compareTo(pivo.getNome()) < pivo.getNome().compareTo(filmeI.getNome())){
                    comparacoes++;
                    celulaI=celulaI.prox;
                    i++;
                }
    
                while (filmeJ.getNome().compareTo(pivo.getNome()) > pivo.getNome().compareTo(filmeI.getNome())){
                    comparacoes++;
                    celulaJ=celulaJ.ant;
                    j--;
            ...
            if (esq < j)
                quicksortDesempate(esq,j);
            if (i < dir)
                quicksortDesempate(i, dir);
        }
    
      
    
        public void setComparacoes(int i) {
            this.comparacoes = i;
        }
        ...
        public void setMovimentacoes(int i) {
            this.movimentacoes = i;
        }
    ...
    public static void main(String[] args) throws Exception{
        ...
        //ListaDupla listaFilmes=new ListaDupla();
        ListaDupla listaFilmes=new ListaDupla();
        ...
        listaFilmes.quicksort();
        listaFilmes.quicksortDesempate();
        listaFilmes.mostrar();
        Arq.openWrite("752959_quicksort2.txt");
        
        listaFilmes.setMovimentacoes(listaFilmes.getMovimentacoes());
        listaFilmes.setComparacoes(listaFilmes.getComparacoes());
        Arq.println("752959\t"+(System.currentTimeMillis()-tempoIni)+" ms\t"+ listaFilmes.getComparacoes()+ "\t"+listaFilmes.getMovimentacoes()); 
        Arq.close();  
    }
